...
2. **与网格线 / 边框 / 背景的绘制次序：维持「覆盖式」（溢出文本直接盖走廊内共享网格线与空格背景），不引入跳线 / 擦线。**
   Luckysheet 跳线（§2.2）与 Univer 擦线（§3.3）都要求绘制侧感知跨格走廊状态，与引擎「节点自治绘制 + 共享边逐格裁决」（`effectiveBorder`，`list-table-scene.ts:458-486`）冲突；覆盖式下网格线 / 空格背景 / 边框按既有次序画即可，溢出文本靠 z 序不变量（取舍 3）保证在上层。空格背景不阻断溢出（§1#3），文本画在其上，对齐 Excel。
