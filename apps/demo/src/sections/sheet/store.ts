...
/** 主 sheet（Sheet 1）：样式矩阵 + 合并区 + 填充演示 + 公式演示格 */
export function createMainStore(): SheetStore {
  ...
  // 首列宽度对齐原列级 width 110
  store.setColWidth(0, 110)
  // 对齐演示行加高（原演示的 setRowHeight(3,48)，尺寸事实源入 Store）
  store.setRowHeight(3, 48)
  ...
  store.setStyle(2, 0, { color: '#646a73' })
  return store
}

/** 次 sheet（Sheet 2）：轻量数字网格（验证切换状态隔离） */
