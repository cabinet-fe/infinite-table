...
import { createRangeRef, parseCellRef, type CellRef } from './address'
...
class Parser {
  ...
  private parsePrimary(): AstNode {
    ...
    if (!token) {
      throw new FormulaParseError('表达式不完整')
    }
    ...
    if (token.type === 'ident') {
      ...
      const ref = parseCellRefLimited(name)
      if (ref) {
        return this.parseRangeTail(ref)
      }
  ...
  private parseCellRefOrRange(sheet: string): AstNode {
    const token = this.next()
    const ref = token?.type === 'ident' ? parseCellRefLimited(token.name) : null
    if (!ref) {
  ...
  /** 单元格引用之后的可选区域后缀（:B9） */
  private parseRangeTail(ref: CellRef): AstNode {
    if (!this.peekOp(':')) {
      return { kind: 'cell', ref }
    }
    this.pos++
    const token = this.next()
    const end = token?.type === 'ident' ? parseCellRefLimited(token.name) : null
    if (!end) {
      throw new FormulaParseError(`区域 ":" 后应为单元格引用，得到 ${describeToken(token)}`)
    }
    return { kind: 'range', ref: createRangeRef(ref, end) }
  }
}

/** 按单元格引用形态解析（列限 3 字母）；不匹配返回 null */
function parseCellRefLimited(name: string): CellRef | null {
  if (!CELL_REF_RE.test(name)) {
    return null
  }
  return parseCellRef(name)
}
